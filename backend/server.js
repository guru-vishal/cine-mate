...
async function testTMDBIntegration() {
  ...
  try {
    ...
    if (popularCount > 0) {
      console.log(
        `   📽️  Sample: "${popular.results?.[0]?.title || popular[0]?.title}"`
      );
    }
    ...
    const actionCount =
      actionMovies.results?.length || actionMovies.length || 0;
    console.log(`✅ Action Movies: ${actionCount} movies fetched`);
    if (actionCount > 0) {
      console.log(
        `   🎬 Sample: "${
          actionMovies.results?.[0]?.title || actionMovies[0]?.title
        }"`
      );
    }
    ...
    const searchCount =
      searchResults.results?.length || searchResults.length || 0;
    console.log(`✅ Search Results: ${searchCount} movies found for "Marvel"`);
    if (searchCount > 0) {
      console.log(
        `   🔍 Sample: "${
          searchResults.results?.[0]?.title || searchResults[0]?.title
        }"`
      );
    }
